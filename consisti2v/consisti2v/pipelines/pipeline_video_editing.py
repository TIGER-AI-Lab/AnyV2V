...
class ConditionalVideoEditingPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        ddim_init_latents_t_idx: Optional[int] = 0,
        **kwargs,
    ):
    ...
    def invert(
        ...
        first_frame_paths: Optional[Union[str, List[str]]] = None,
        first_frames: Optional[torch.FloatTensor] = None,
        noise_sampling_method: str = "pyoco_mixed",
        noise_alpha: float = 1.0,
        guidance_rescale: float = 0.0,
        frame_stride: Optional[int] = None,
        use_frameinit: bool = False,
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs).prev_sample
                save_latents = latents.detach().clone()
                save_latents = torch.cat([first_frame_latents.unsqueeze(2), save_latents], dim=2)

                inverted_latents.append(save_latents)  # Modified
        ...
        if output_type == "latent":
            return AnimationPipelineOutput(videos=inverted_latents)
    ...
    @torch.no_grad()
    def sample_with_saving_features(
        self,
        prompt: Union[str, List[str]],
        video_length: Optional[int],
        height: Optional[int] = None,
                ...
                if first_frame_latents is not None:
                    ...
                    for res in up_res_dict:
                        for block in up_res_dict[res]:
                            module = self.unet.up_blocks[res].resnets[block]
                            setattr(module, "t", t.item())
                            if res != 0:
                                module = self.unet.up_blocks[res].attentions[block].transformer_blocks[0].attn1.processor
                                setattr(module, "t", t.item())
                                module = self.unet.up_blocks[res].tempo_attns[block].transformer_blocks[0].attn1.processor
                                setattr(module, "t", t.item())
    ...
    def encode_vae_video(
        ...
        for i in range(0, n_frames):
            ...
            frame = frame.to(device=device, dtype=dtype).unsqueeze(0)
    ...
    def sample_with_pnp(
        ...
        ddim_inv_1st_frame_path: Optional[Union[str, List[str]]] = None, # [Modified]
        **kwargs,
    ):
        if first_frame_paths is not None and first_frames is not None:
            raise ValueError("Only one of `first_frame_paths` and `first_frames` can be passed.")
        ...
        ddim_inv_prompt_embeds = self._encode_prompt(
            ddim_inv_prompt, device, num_videos_per_prompt, do_classifier_free_guidance=False, negative_prompt=None
        )

        # copied from _encode_prompt, don't uncomment
        # if do_classifier_free_guidance == "text":
        #     text_embeddings = torch.cat([uncond_embeddings, text_embeddings])
        # elif do_classifier_free_guidance == "both":
        #     text_embeddings = torch.cat([uncond_embeddings, uncond_embeddings, text_embeddings])

        text_embeddings = torch.cat([ddim_inv_prompt_embeds, text_embeddings])
        ...
        # Encode PnP first frame
        ddim_inv_1st_frame_latents = None
        if ddim_inv_1st_frame_path is not None:
            ddim_inv_1st_frame_path = ddim_inv_1st_frame_path if isinstance(ddim_inv_1st_frame_path, list) else [ddim_inv_1st_frame_path] * batch_size
            if camera_motion is None:
                img_transform = T.Compose([
                    T.ToTensor(),
                    T.Resize((height, width), antialias=None),
            ...
            ddim_inv_1st_frames = []
            for ddim_inv_1st_frame_path in ddim_inv_1st_frame_path:
                ddim_inv_1st_frame = Image.open(ddim_inv_1st_frame_path).convert('RGB')
                ddim_inv_1st_frame = img_transform(ddim_inv_1st_frame)
                if camera_motion is not None:
                    if camera_motion == "pan_left":
                        ddim_inv_1st_frame = pan_left(ddim_inv_1st_frame, num_frames=video_length, crop_width=width)
                    elif camera_motion == "pan_right":
                        ddim_inv_1st_frame = pan_right(ddim_inv_1st_frame, num_frames=video_length, crop_width=width)
                    elif camera_motion == "zoom_in":
                        ddim_inv_1st_frame = zoom_in(ddim_inv_1st_frame, num_frames=video_length, crop_width=width)
                    elif camera_motion == "zoom_out":
                        ddim_inv_1st_frame = zoom_out(ddim_inv_1st_frame, num_frames=video_length, crop_width=width)
                    else:
                        raise NotImplementedError(f"camera_motion: {camera_motion} is not implemented.")
                ddim_inv_1st_frames.append(first_frame.unsqueeze(0))
            ddim_inv_1st_frames = torch.cat(ddim_inv_1st_frames, dim=0)
        if ddim_inv_1st_frames is not None:
            ddim_inv_1st_frames = ddim_inv_1st_frames.to(device, dtype=self.vae.dtype)
            if camera_motion is not None:
                ddim_inv_1st_frames = rearrange(ddim_inv_1st_frames, "b f c h w -> (b f) c h w")
            ddim_inv_1st_frame_latents = self.vae.encode(ddim_inv_1st_frames).latent_dist
            ddim_inv_1st_frame_latents = ddim_inv_1st_frame_latents.sample()
            ddim_inv_1st_frame_latents = ddim_inv_1st_frame_latents * self.vae.config.scaling_factor # b, c, h, w
            ddim_inv_1st_frame_static_vid = rearrange(ddim_inv_1st_frame_latents, "(b f) c h w -> b c f h w", f=video_length if camera_motion is not None else 1)
            ddim_inv_1st_frame_latents = ddim_inv_1st_frame_static_vid[:, :, 0, :, :]
            ddim_inv_1st_frame_latents = repeat(ddim_inv_1st_frame_latents, "b c h w -> (b n) c h w", n=num_videos_per_prompt)
            ddim_inv_1st_frames = repeat(ddim_inv_1st_frames, "b c h w -> (b n) c h w", n=num_videos_per_prompt)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ddim_inv_latents_at_t = ddim_inv_latents_at_t[:, :, 1:, :, :]
                # expand the latents if we are doing classifier free guidance
                if do_classifier_free_guidance is None:
                    latent_model_input = torch.cat([ddim_inv_latents_at_t, latents])
                elif do_classifier_free_guidance == "text":
                    latent_model_input = torch.cat([ddim_inv_latents_at_t] + [latents] * 2)
                elif do_classifier_free_guidance == "both":
                    latent_model_input = torch.cat([ddim_inv_latents_at_t] + [latents] * 3)
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                if first_frame_latents is not None:
                    if do_classifier_free_guidance is None:
                        first_frame_latents_input = torch.cat([ddim_inv_1st_frame_latents, first_frame_latents])
                    elif do_classifier_free_guidance == "text":
                        first_frame_latents_input = torch.cat([ddim_inv_1st_frame_latents] + [first_frame_latents] * 2)
                    elif do_classifier_free_guidance == "both":
                        first_frame_latents_input = torch.cat([ddim_inv_1st_frame_latents] + [first_frame_noisy_latent, first_frame_latents, first_frame_latents])
                ...
                if do_classifier_free_guidance:
                    if do_classifier_free_guidance == "text":
                        _noise_pred_ddim_inv, noise_pred_uncond, noise_pred_text = noise_pred.chunk(3)
                        noise_pred = noise_pred_uncond + guidance_scale_txt * (noise_pred_text - noise_pred_uncond)
                    elif do_classifier_free_guidance == "both":
                        _noise_pred_ddim_inv, noise_pred_uncond, noise_pred_img, noise_pred_both = noise_pred.chunk(4)
                        noise_pred = noise_pred_uncond + guidance_scale_img * (noise_pred_img - noise_pred_uncond) + guidance_scale_txt * (noise_pred_both - noise_pred_img)
                else:
