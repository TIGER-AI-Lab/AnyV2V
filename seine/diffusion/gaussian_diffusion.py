...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None,
        ...
        B, F, C = x.shape[:3]
        # print(f"[DEBUG-INFO]: x.shape: {x.shape}, t.shape: {t.shape}, mask.shape: {mask.shape}, x_start.shape: {x_start.shape}")
        # print(f"[DEBUG-INFO]: use_concat: {use_concat}")
        # print(f"[DEBUG-INFO]: model_kwargs.keys(): {model_kwargs.keys()}")
        # print(f"[DEBUG-INFO]: th.concat([x, mask, x_start], dim=1).shape: {th.concat([x, mask, x_start], dim=1).shape}")
        assert t.shape == (B,)
        if use_concat:
